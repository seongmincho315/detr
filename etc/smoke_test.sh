#!/usr/bin/env bash
# DETR layout server smoke test entrypoint.
#
# 컨테이너 내부에서 실행한다.
#
# 사용법:
...
#   SMOKE_OUT_DIR       결과 JSON 저장 디렉토리 (기본 /tmp/detr_smoke)
set -euo pipefail

PORT="${DETR_PORT:-8080}"
TIMEOUT="${HEALTH_TIMEOUT_SEC:-120}"
OUT_DIR="${SMOKE_OUT_DIR:-/tmp/detr_smoke}"
APP_DIR="${APP_DIR:-/app}"
...
cmd_inference() {
  echo "[smoke] running in-process layout inference"
